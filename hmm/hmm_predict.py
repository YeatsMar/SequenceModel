...
def divide_sentence(hmm_model_file_path, sentence):   # todo: sentence_list contains many sentences
    # 0 read model file
    ...
    # 2 Forward: calculate weight and path matrix for word partition
...
def predict(hmm_model_file_path, test_file):
    # read test file
    ...
    status, input_sentence_list, all_state_list = file_tool.read_training_lines(lines)
    # 读出来的是去掉空格和标点的多句分句，每句分句里也都是char list
    if not status:
    ...
    for input_sentence in input_sentence_list:
        status, response = divide_sentence(hmm_model_file_path, input_sentence)
        if not status:
            print "Error in hmm main divide_sentence ", response
        result_list.append(response)

    return True, result_list, all_state_list, input_sentence_list
...
def __calculate_weight_and_path(init_state_prob, prob_trans_matrix, prob_emit_maps, input_sentence):
    sentence_length = len(input_sentence) + 1    # todo: last transition  [sentence_length+1]
    state_num = global_varibale.state_count
    score_matrix = np.zeros((state_num, sentence_length))
    ...
    for i in range(state_num):
        ...
        if not emit_map_of_this_state.has_key(start_char):
            score_matrix[i][char_index] = init_state_prob[i] + global_varibale.min_number   # todo: + means probability multiplication
        else:
            score_matrix[i][char_index] = init_state_prob[i] + emit_map_of_this_state[start_char]

    # calculate all remains for each char in the sentence
    for i in range(1, sentence_length):     # todo: last transition  [sentence_length+1]
        # for all the states
        for state in range(state_num):
            score_matrix[state][i] = 0 - sys.float_info.max
            ...
            for k in range(state_num):
                ...
                prev_char_weight = score_matrix[k][i - 1]
                ...
                if tmp > score_matrix[state][i]:
                    score_matrix[state][i] = tmp
                    path_matrix[state][i] = k

    for state in range(4):
        score_matrix[state][sentence_length-1] = 0 - sys.float_info.max

    return True, score_matrix, path_matrix
...
def __trace_back(state_array, weight_matrix, path_matrix, sentence):
    ...
    last_char_index = len(sentence)
    label_num = len(state_array)    # 一共有几个标签，系统默认的标签列表
    ...
    # 返回回溯的结果
    # output_state_array = [state_array[last_char_label_index]]  todo: comment because of EOS
    ...
    try:
        ...
        print state_array, weight_matrix, path_matrix, sentence
        exit(0)
    return True, track_back_string
